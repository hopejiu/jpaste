...
export default function ShortcutHelpModal({ open, onClose }) {
  ...
  return (
    ...
    >
      <div
        className="bg-elevated border border-border rounded-lg shadow-glass-lg w-[420px] max-w-[90vw] max-h-[80vh] flex flex-col animate-[slideUp_200ms_ease-out]"
        onClick={e => e.stopPropagation()}
      >
        {/* Header */}
        <div className="flex items-center justify-between px-[18px] py-3.5 border-b border-border flex-shrink-0">
          <h3 className="m-0 text-base font-semibold text-foreground">快捷键说明</h3>
          <button
        ...
        {/* Body */}
        <div className="p-[18px] overflow-auto flex-1 space-y-4">
              ...
              <div className="flex flex-col gap-1">
                {group.items.map(item => (
                  <div key={item.keys} className="flex items-baseline gap-2 text-xs">
                    <kbd
                      className="px-1.5 py-[2px] rounded font-mono font-medium flex-shrink-0"
                      style={{
